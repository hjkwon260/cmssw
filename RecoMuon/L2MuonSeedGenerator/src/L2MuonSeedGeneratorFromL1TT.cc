/*  \class L2MuonSeedGeneratorFromL1TT
 *
 *   L2 muon seed generator:
 *   Transform the L1TTon informations in seeds
...
// constructors
L2MuonSeedGeneratorFromL1TT::L2MuonSeedGeneratorFromL1TT(const edm::ParameterSet &iConfig)
  ...
  muCollToken_ = consumes<std::vector<TTTrack<Ref_Phase2TrackerDigi_>>>(theSource);
...
// destructor
L2MuonSeedGeneratorFromL1TT::~L2MuonSeedGeneratorFromL1TT() {
  if (theService)
    delete theService;
  if (theEstimator)
    delete theEstimator;
...
void L2MuonSeedGeneratorFromL1TT::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  descriptions.add("L2MuonSeedGeneratorFromL1TT", desc);
}

void L2MuonSeedGeneratorFromL1TT::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  const std::string metname = "Muon|RecoMuon|L2MuonSeedGeneratorFromL1TT";
  MuonPatternRecoDumper debug;

  auto output = std::make_unique<L2MuonTrajectorySeedCollection>();

  edm::Handle<std::vector<TTTrack< Ref_Phase2TrackerDigi_>>> muColl;
  ...
  for (auto it = muColl->begin(); it != muColl->end(); it++) {
...
const TrajectorySeed *L2MuonSeedGeneratorFromL1TT::associateOfflineSeedToL1(
    edm::Handle<edm::View<TrajectorySeed>> &offseeds,
    std::vector<int> &offseedMap,
    TrajectoryStateOnSurface &newTsos,
    double dRcone) {
  const std::string metlabel = "Muon|RecoMuon|L2MuonSeedGeneratorFromL1TT";
