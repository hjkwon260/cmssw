//-------------------------------------------------
//
/**  \class L2MuonSeedGeneratorFromL1THJ
 * 
...
bool sortByL1PtHJ(L2MuonTrajectorySeed &A, L2MuonTrajectorySeed &B) {
  l1t::MuonRef Ref_L1A = A.l1tParticle();
  l1t::MuonRef Ref_L1B = B.l1tParticle();
  return (Ref_L1A->pt() > Ref_L1B->pt());
};

bool sortByL1QandPtHJ(L2MuonTrajectorySeed &A, L2MuonTrajectorySeed &B) {
  l1t::MuonRef Ref_L1A = A.l1tParticle();
  l1t::MuonRef Ref_L1B = B.l1tParticle();
...
// constructors
L2MuonSeedGeneratorFromL1THJ::L2MuonSeedGeneratorFromL1THJ(const edm::ParameterSet &iConfig)
  ...
  t_L1TT = consumes<std::vector< TTTrack< Ref_Phase2TrackerDigi_>>>(theSource2);
...
// destructor
L2MuonSeedGeneratorFromL1THJ::~L2MuonSeedGeneratorFromL1THJ() {
  if (theService)
    delete theService;
  if (theEstimator)
    delete theEstimator;
...
void L2MuonSeedGeneratorFromL1THJ::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  descriptions.add("L2MuonSeedGeneratorFromL1THJ", desc);
}

void L2MuonSeedGeneratorFromL1THJ::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  const std::string metname = "Muon|RecoMuon|L2MuonSeedGeneratorFromL1THJ";
 ...
 // std::cout<< "Number of muons " << muColl->size() << std::endl;
  ...
  iEvent.getByToken(t_L1TT, h_L1TT);
  ...
  if (matchType == 0) {
    ...
    // for (auto it = h_L1TT->begin(); it != h_L1TT->end(); it++) {
    ...
    //     continue;
      // for (auto it = muColl->begin(ibx); it != muColl->end(ibx); it++) {
        // cout << "L1TT size: " << h_L1TT->size() << endl;
  ...
  //--- SortType 1 : by L1 pT
  if (sortType == 1) {
    sort(output->begin(), output->end(), sortByL1PtHJ);
  }

  //--- SortType 2 : by L1 quality and pT
  else if (sortType == 2) {
    sort(output->begin(), output->end(), sortByL1QandPtHJ);
  }
...
const TrajectorySeed *L2MuonSeedGeneratorFromL1THJ::associateOfflineSeedToL1(
    edm::Handle<edm::View<TrajectorySeed>> &offseeds,
    std::vector<int> &offseedMap,
    TrajectoryStateOnSurface &newTsos,
    double dRcone) {
  const std::string metlabel = "Muon|RecoMuon|L2MuonSeedGeneratorFromL1THJ";
...
bool L2MuonSeedGeneratorFromL1THJ::isAssociateOfflineSeedToL1(
    edm::Handle<edm::View<TrajectorySeed>> &offseeds,
